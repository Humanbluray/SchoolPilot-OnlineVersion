...
class NotesPrim(ft.Tab):
    def __init__(self, cp: object):
        super(NotesPrim, self).__init__(
            tab_content=ft.Row(
                controls=[
                    ft.Icon(ft.icons.NOTE_ALT_ROUNDED, size=20),
                    ft.Text("Niveaux 6e/5e".upper(), font_family="Poppins Medium", size=12)
                ]
            )
        )
        self.cp = cp
        ...
        self.table = ft.DataTable(
            columns=[
                ...
                ft.DataColumn(label=ft.Text("Compétence")),
                    ...
                    ft.Container(
                        ...
                        content=ft.Column(
                            ...
                            controls=[
                                ft.Row(
                                    controls=[
                                        ft.Text("Notes (6e et 5e)".upper(), size=13, font_family="Poppins Medium"),
                                        ft.Row(
                                            controls=[
                                                AnyButton(
    ...
    def filter_datas(self, e):
        datas = be.show_all_notes_prim()

        notes = []
        for data in datas:
            dico = {
                'eleve': data[1], 'asco': data[2], 'sequence': data[3], 'classe': data[4],
                'matiere': data[5], "competence": data[6], 'coeff': data[7], 'note': data[8], 'id': data[9]
            }
            notes.append(dico)
            ...
            self.table.rows.append(
                ft.DataRow(
                    ...
                    cells=[
                        ...
                        ft.DataCell(ft.Text(note['competence'])),
    ...
    def load_lists(self):
        ...
        for my_class in classes:
            if be.look_nivo(my_class) == "6E" or be.look_nivo(my_class) == "5E":
                self.classe.options.append(ft.dropdown.Option(my_class))
                self.imp_classe.options.append(ft.dropdown.Option(my_class))
                self.new_classe.options.append(ft.dropdown.Option(my_class))
    ...
    def on_change_classe(self, e):
        my_class = self.classe.value if self.classe.value is not None else ""

        if my_class == "":
            pass
        else:
            level = be.niv_fn_classe(self.classe.value)
            subjects = be.show_matieres_fn_niv(level)

            for any_subject in self.matiere.options[:]:
                self.matiere.options.remove(any_subject)

            for subject in subjects:
                self.matiere.options.append(
                    ft.dropdown.Option(subject.upper())
                )

            self.matiere.update()

    def on_change_imp_classe(self, e):
        my_class = self.imp_classe.value if self.imp_classe.value is not None else ""

        if my_class == "":
            pass
        else:
            level = be.niv_fn_classe(self.imp_classe.value)
            subjects = be.show_matieres_fn_niv(level)

            for subject in self.imp_matiere.options[:]:
                self.imp_matiere.options.remove(subject)
    ...
    def on_change_imp_matiere(self, e):
        level = be.look_nivo(self.imp_classe.value)
        coeff = be.coef_fn_mat_niv(self.imp_matiere.value, level)
