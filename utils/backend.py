...
my_base = "utils/Ecole.db"

""" 1. Fonctions générales ________________________________________"""
...
def connection_base():
    ...
    # Années scolaires
    c.execute("""CREATE TABLE IF NOT EXISTS annee_scolaire(
    ...
    # niveaux
    c.execute("""CREATE TABLE IF NOT EXISTS niveaux(
                niv_id        INTEGER PRIMARY KEY AUTOINCREMENT,
                niv_nom       TEXT,
                niv_exam      TEXT,
                niv_name_exam TEXT,
                niv_serie     TEXT,
    ...
    # Notes autres ...
    c.execute("""CREATE TABLE IF NOT EXISTS Notes_prim(
                    id               INTEGER PRIMARY KEY AUTOINCREMENT,
                    nom              TEXT,
                    asco             INTEGER,
                    sequence         TEXT,
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS inscriptions(
                ins_num       INTEGER PRIMARY KEY AUTOINCREMENT,
                ins_asco      INTEGER,
                ins_eleve     TEXT,
                ins_matricule TEXT,
                ins_classe    TEXT,
    ...
    # Pensions
    c.execute("""CREATE TABLE IF NOT EXISTS pensions(
                pens_id INTEGER PRIMARY KEY AUTOINCREMENT,
                asco    NUMERIC,
                elv     TEXT,
                tranche TEXT,
                montant NUMERIC,
    ...
    # Tranches
    c.execute("""CREATE TABLE IF NOT EXISTS tranches(
    ...
    # Trimestres
    c.execute("""CREATE TABLE IF NOT EXISTS trimestres (           
                id_tri   INTEGER PRIMARY KEY AUTOINCREMENT,
                trim_nom TEXT
                trim_seq TEXT)""")

    # profs tittulaires
    c.execute(""" CREATE TABLE IF NOT EXISTS prof_titus (
                professeur TEXT,
                classe     TEXT,
                asco       NUMERIC )""")
    ...
    # Affectations
    c.execute("""CREATE TABLE affectations (
                    id        INTEGER PRIMARY KEY AUTOINCREMENT,
                    asco      NUMERIC,
                    prof      TEXT,
                    classe    TEXT,
    ...
    # sanctions
    c.execute("""CREATE TABLE sanctions (
                    id        INTEGER PRIMARY KEY AUTOINCREMENT,
                    asco      NUMERIC,
                    nom       TEXT,
                    matricule TEXT,
...
""" Fin des Fonctions générales ________________________________________"""


""" 2. Fonctions de la table années scolaires ________________________________________"""


# 2. Fonctions de la table années scolaires
def find_last_date():
    conn = sqlite3.connect(my_base)
    cur = conn.cursor()
...
def all_affectations_by_annee():
    ...
    res = cur.fetchall()
    final = [
        {'asco': row[1], 'prof': row[2], 'classe': row[3], 'matiere': row[4], 'jour': row[6], 'nb_heures': row[5], "creneau": row[7]}
        for row in res
    ]
    con.commit()
    con.close()
...
def add_affectation(prof, classe, matiere, heure, jour, creneau):
    ...
    cur = conn.cursor()
    asco = show_asco_encours()
    cur.execute("""INSERT INTO affectations values (?,?,?,?,?,?,?,?)""", (cur.lastrowid, asco, prof, classe, matiere, heure, jour, creneau))
    conn.commit()
    conn.close()
...
def search_prof_affec(classe, matiere):
    ...
    cur = conn.cursor()
    asco = show_asco_encours()
    cur.execute("""SELECT prof FROM affectations WHERE asco = ? AND classe = ? AND aff_mat = ?""", (asco, classe, matiere))
    res = cur.fetchone()
    conn.commit()
    conn.close()
    return res[0]
...
def is_creneau_prof_oqp(prof, jour, creneau):
    ...
    cur = conn.cursor()
    asco = show_asco_encours()
    cur.execute("""SELECT * FROM affectations  WHERE prof = ? AND jour = ? AND creneau = ? AND asco = ?""", (prof, jour, creneau, asco))
    res = cur.fetchone()
    conn.commit()
    conn.close()
    return True if res is not None else False
...
def is_creneau_classe_oqp(classe, jour, creneau):
    ...
    cur = conn.cursor()
    asco = show_asco_encours()
    cur.execute("""SELECT * FROM affectations  WHERE classe = ? AND jour = ? AND creneau = ? AND asco = ?""", (classe, jour, creneau, asco))
    res = cur.fetchone()
    conn.commit()
    conn.close()
...
def is_creneau_occupe(prof, jour, creneau):
    ...
    cur = conn.cursor()
    asco = show_asco_encours()
    cur.execute("""SELECT * FROM affectations classe = ? and jour = ? and creneau = ? and asco =?""", (prof, jour, creneau, asco))
    res = cur.fetchone()
    conn.commit()
    conn.close()
    return True if res is not None else False
...
def update_elev(nom, date, lieu, sexe, pere, mere, con, id_elev):
    conn = sqlite3.connect(my_base)
    c = conn.cursor()
    c.execute("""UPDATE eleves SET
                ...
                mere = ?,
                contact = ? 
                WHERE id = ?""", (nom, date, lieu, sexe, pere, mere, con, id_elev))
    conn.commit()
    conn.close()
...
def add_eleve(nom, date, lieu, sexe, pere, mere, contact, matricule):
    conn = sqlite3.connect(my_base)
    c = conn.cursor()
    param = (c.lastrowid, nom, date, lieu, sexe, pere, mere, contact, matricule)
...
def sanction_by_eleve_seq(nom, sequence, typp):
    ...
    asco = show_asco_encours()
    c = conn.cursor()
    c.execute("SELECT sum(qte) FROM sanctions WHERE asco = ? AND nom = ? AND sequence = ? AND type = ?", (asco, nom, sequence, typp))
    res = c.fetchone()
    conn.commit()
    conn.close()
    if res[0] is None:
...
def global_pension():
    ...
    cur.execute("""SELECT ins_asco, ins_eleve, ins_classe, 
                    ...
                    (SELECT sum(tranche_mt) FROM tranches WHERE pension = "oui" and asco = ?) as total

                    FROM inscriptions WHERE ins_asco = ? ORDER BY ins_classe""", (show_asco_encours(), show_asco_encours(), show_asco_encours()))
...
def add_user(nom, poste, acces, login):
    con = sqlite3.connect(my_base)
    cur = con.cursor()
    cur.execute("""INSERT INTO users values (?,?,?,?,?,?,?)""", (cur.lastrowid, nom, poste, acces, login, "", "EN ATTENTE"))
    con.commit()
    con.close()
